...
| `kitty-multi-cursor` | Whether to emit extra cursor positions using the Kitty multi-cursor protocol. Can be `auto`, `enabled` or `disabled`. On Windows, `auto` cannot detect support; use `enabled` with a supporting terminal. | `"auto"` |
